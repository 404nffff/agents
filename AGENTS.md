...
- 需要上下文收集，或需要根据接口地址/路由路径定位代码时（例如 `/admin/student/export_student`），必须先调用 `mcp__codebase-retrieval__codebase-retrieval`；若失败需重试 3 次，3 次仍无法使用或无结果时，再降级到 `mcp__fast-context__fast_context_search`；若仍无法使用再降级到 `mcp__code-index__*`
...
| `mcp__codebase-retrieval__codebase-retrieval` 执行失败或无结果时，必须先重试 3 次；仅当 3 次仍失败或无结果时，才允许降级到下一检索工具 | 强制重试，不得跳过 |
| 当任务以接口地址或路由路径定位代码（例如 `/admin/student/export_student`）时，必须先调用 `mcp__codebase-retrieval__codebase-retrieval` 并遵循“失败重试 3 次”规则；仅在 3 次仍失败或无结果时，才可降级到 `mcp__fast-context__fast_context_search`，再降级到 `mcp__code-index__*`、`rg` 或 Read | 强制前置，禁止跳步 |
...
| 内部代码或文档检索优先使用 `mcp__codebase-retrieval__codebase-retrieval`；若该工具不可用、未注入、未调通，或单次无结果，则必须重试 3 次；3 次仍失败或无结果时，降级到 `mcp__fast-context__fast_context_search`；若仍不可用或结果不足，再降级为 `mcp__code-index__*`、`rg` 或 Read，并在日志中声明 | 保持检索工具一致性 |
...
- 技术栈：使用的框架、语言、关键依赖，优先使用 `mcp__codebase-retrieval__codebase-retrieval`；若失败或无结果需重试 3 次，仍失败或无结果再降级到 `mcp__fast-context__fast_context_search`
- 若输入包含接口地址或路由路径（例如 `/admin/student/export_student`），必须先使用 `mcp__codebase-retrieval__codebase-retrieval` 定位代码文件和行号；若失败或无结果需重试 3 次，3 次仍失败或无结果再降级到 `mcp__fast-context__fast_context_search`，若仍无法使用再降级到 `mcp__code-index__*` 或 `rg`
- 测试：现有测试文件和验证方式
...
#### 步骤5：修改日志收集（可选）
- 如果指定了plan/下的*.md文件，优先阅读该文件，并根据文件内容进行处理。并在完成后 记录当前修改的日志，记录到plan/下的*.md文件中。
...
| 读取必须优先使用 `mcp__codebase-retrieval__codebase-retrieval`，失败或无结果时必须重试 3 次；3 次仍失败或无结果才可降级到 `mcp__fast-context__fast_context_search`，若仍无法使用再降级到 `mcp__code-index__*`、Read、Grep 等检索接口 | |
...
| 网络搜索必须先执行 `grok-search` skill；若其不可用、未配置、执行失败或结果不足，则降级到 exa；再次失败后才可使用其他搜索工具。内部检索优先 `mcp__codebase-retrieval__codebase-retrieval`；若其不可用、未注入、未调通，或单次无结果则必须重试 3 次；3 次仍失败或无结果才降级到 `mcp__fast-context__fast_context_search`；若仍不可用或结果不足再降级到 `mcp__code-index__*`、`rg` 或 Read。若输入为接口地址/路由路径（如 `/admin/student/export_student`），必须按该重试与降级顺序执行；深度思考必用 sequential-thinking | 工具优先级规范 |
...
- 构建和编译操作
- 工具调用（`grok-search` skill、`mcp__codebase-retrieval__codebase-retrieval`、`mcp__fast-context__fast_context_search`、`mcp__code-index__*`、exa、grep、find 等）
- 按计划执行的所有步骤
- 错误修复和重试（最多3次）
