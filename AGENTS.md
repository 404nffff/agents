...
- 需要上下文收集时，优先显式调用 `mcp__fast-context__fast_context_search`
...
| 内部代码或文档检索优先使用 `mcp__fast-context__fast_context_search`；若当前会话未注入该工具或返回结果不足，则依次降级为 `mcp__code-index__*`、`rg` 或 Read，并在日志中声明 | 保持检索工具一致性 |
...
| 读取必须优先使用 `mcp__fast-context__fast_context_search`，其次使用 `mcp__code-index__*`、Read、Grep 等检索接口 | |
...
| 网络搜索必须先执行 `grok-search` skill；若其不可用、未配置、执行失败或结果不足，则降级到 exa；再次失败后才可使用其他搜索工具。内部检索优先 `mcp__fast-context__fast_context_search`；若其不可用或结果不足则降级到 `mcp__code-index__*`、`rg` 或 Read；深度思考必用 sequential-thinking | 工具优先级规范 |
