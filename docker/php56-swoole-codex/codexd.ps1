...
$Container = $env:CODEX_DOCKER_CONTAINER
$HostRoot = $env:CODEX_HOST_ROOT
$ContainerRoot = $env:CODEX_CONTAINER_ROOT
$InstallPath = $false
$PrintPath = $false
$DryRun = $false
$CodexArgs = @()
...
if ($LASTEXITCODE -ne 0) {
  throw "容器不存在: $Container，可用 CODEX_DOCKER_CONTAINER 指定容器名。"
}
if ($running -ne "true") {
...
$codexCommand = "cd $quotedContainerPath && exec codex --no-alt-screen --yolo"
if ($quotedArgs.Count -gt 0) {
  $codexCommand += " " + ($quotedArgs -join " ")
}

if ($DryRun) {
  Write-Output ("docker exec {0} zsh -ic {1}" -f $Container, (Quote-Sh $codexCommand))
  exit 0
}
...
$execArgs += @($Container, "zsh", "-ic", $codexCommand)
