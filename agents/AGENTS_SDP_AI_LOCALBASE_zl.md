...
*   **工作模式 (指令驱动 SDLC + 阶段选角 + 链路验证 + 默认代码优先)**：严格遵循标准化的软件开发生命周期，具体执行特征如下：
    ...
    *   **阶段选角强制执行**：每进入一个 SDLC 阶段，必须先依据当前阶段目标与 Skill `skills/who/SKILL.md` 的选择规则确定 1 个主角色；只有天然跨域时才允许补 1 个辅助角色。禁止一次性读取整套 `skills/who/roles/` 角色库。
...
**流程执行由 Skill `software-dev-process-zl` 负责，阶段选角由 `skills/who/SKILL.md` 辅助执行**。详细的阶段定义、模板引用、PHP 链路探针验证、阶段边界和角色选择规则请参考对应 Skill 文档。

### 阶段 1：需求理解与上下文收集

1. **指令触发**：必须通过 `sdlc-design-1`, `sdlc-design-2`, `sdlc-implement`, `sdlc-test`, `sdlc-debug`, `sdlc-solo` 触发对应阶段。
2. **阶段选角起步**：进入阶段后，先依据当前阶段目标与 `skills/who/SKILL.md` 选择当前阶段主角色；仅在天然跨域时补 1 个辅助角色。
...
2. **阶段内重新选角**：进入本阶段后，必须先按当前任务或任务组目标重新确定主角色；只有天然跨域时才补 1 个辅助角色。
...
5. **并行与派发约束**：若准备并行执行多个任务，必须先确认任务之间无共享改动文件、无顺序依赖、无接口契约互相等待，并把“允许/不允许、原因、并行任务组、主控决定”写入 `003-施工文档.md`；未满足条件一律串行。派发子代理时必须组装最小上下文，严禁越界读取或修改。
...
13. **禁止过度封装 helper 函数**：除非同一段逻辑在本次改动中出现三次以上且施工文档已明确批准抽象，否则禁止新增 helper 函数、工具函数、包装类或中转层；应直接在目标业务代码点完成实现。
14. **新增代码块位置约束**：在既有代码文件中新增独立函数、类、常量块、配置块或测试用例块时，统一追加到该代码文件底部；只有语法、框架注册顺序或编译约束要求必须放在特定位置时，才允许例外，并必须在 `003-文件改动记录.md` 说明原因。
15. **自动执行边界**：在获批范围内可自动执行读写、编码、命令与测试；遇到红线、超范围需求或连续 3 次相同失败时，必须暂停并询问用户。
16. **代码注释强制同步**：所有新增、修改的代码必须在施工阶段同步补齐中文注释，明确实现意图、约束条件和使用方式；禁止把注释补写拖到交付后。
17. **小步修改**：每次变更都必须保持可编译、可验证。
18. **SQL 目录与编写约束**：所有数据库相关产物（DDL、DML、查询语句、修复脚本、回填脚本）必须统一落到 `docs/[任务目录]/sql/`，禁止继续写入 `docs/db/` 或任务目录外位置。编写 SQL 前必须先检查目标表索引，并按索引条件设计 SQL；禁止写事务包装；禁止写 `JOIN`，需要拆分为多条单表查询并在业务代码中组装；禁止使用数据库函数处理业务逻辑、格式化或计算，相关处理必须放到业务代码层。
19. **注释规范**：凡是接口、路由、控制器、Handler 或其他对外提供服务的代码，必须优先遵循所在文件已有的注释规则；若文件内没有可参考的注释规则，再在函数或方法头部使用 Swagger doc 注释规范，至少声明接口用途、请求参数、返回结构、状态码与必要说明，保证生成文档时可直接识别。
20. **数据库查询执行约束**：执行阶段凡涉及 MySQL、Redis、Mongo、PGSQL 查询，必须先使用 `db-query` skill；仅当 MySQL 查询场景下 `db-query` 连续重试 3 次仍不可用、不存在或执行失败时，才允许降级使用 `mysql-query` skill；禁止绕过该流程直接执行数据库查询。凡需产出 SQL，必须先用只读方式检查相关表索引；禁止事务、`JOIN` 和数据库函数处理；多表数据必须拆分查询，按业务链路在代码层组装。

### 阶段 4：质量验证与收尾

1. **阶段测试选角**：进入 `sdlc-test` 或 `sdlc-debug` 前，必须先按阶段目标重新确定主角色；测试阶段主角色通常为 `Code Reviewer` 或 `Senior Developer`，调试阶段主角色通常为 `Senior Developer` 或 `Incident Response Commander`。
