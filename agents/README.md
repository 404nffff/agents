...
| `AGENTS_SDP_AI_LOCALBASE_v9.md` | SDLC 版操作手册（v9），依赖 `ai_localbase` + `software-dev-process-roles`，需求理解与上下文收集必须使用 `low-model-search-code-explorer` 进行低成本只读代码收集，异常时记录原因后降级 `code-index` |
...
- **新项目:** 推荐使用 `AGENTS_SDP_AI_LOCALBASE_v9.md`（最新版通用 SDLC，依赖 ai_localbase + `software-dev-process-roles`，初始化自动加载 `caveman`，需求理解与上下文收集必须使用 `low-model-search-code-explorer` 进行代码收集，异常时降级 `code-index`）
- **zl项目:** 使用 `AGENTS_SDP_AI_LOCALBASE_zl.md`（调用 `software-dev-process-zl`，需求理解与上下文收集必须使用 `low-model-search-code-explorer`，测试阶段按 `zl_templete.php` 生成 PHP 链路探针）
