...
*   **写入方式 (及时沉淀)**：所有文档必须写入当前启动目录对应的知识库中（禁止写入其他知识库）。新文档首次沉淀使用 `document_upload`；已有文档补充增量内容使用 `document_append`；需要整体覆盖修订时使用 `document_update`；废弃文档使用 `document_delete`。修改前必须先读取完整内容。
*   **默认同步规则**：凡是 `docs/[任务目录]/` 下新产出的计划、概要设计、详细设计、施工文档、状态文档、测试文档、排查文档，在对应阶段完成后默认同步到 `ai_localbase` 中对应的知识库，无需用户再次提醒；其中新文档首次入库使用 `document_upload`，已有核心文档整体修订使用 `document_update`，持续追加型文档（如 `operations-log.md`）使用 `document_append`。
...
3. **强制更新项目知识库**：任务完成后必须优先将新增或变更的设计结论、施工结果、测试结论同步到 `ai_localbase`；新文档使用 `document_upload`，已有文档补充使用 `document_append` 或 `document_update`，禁止跳过知识库更新直接结束任务。**同步时必须强制写入当前启动目录对应的知识库中**。
