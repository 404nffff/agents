...
- 若首选 `grok-search` skill 不可用、未配置、执行报错或返回结果不足，才允许按降级策略改用 `exa`、`chrome-devtools` 或其他可用代理进行检索，并在操作日志中标注原因与输出摘要。
- 需要上下文收集，或需要根据接口地址/路由路径定位代码时（例如 `/admin/student/export_student`），必须先调用 `mcp__codebase-retrieval__codebase-retrieval`；若无法使用再降级到 `mcp__fast-context__fast_context_search`；若仍无法使用再降级到 `mcp__code-index__*`
...
遇到以下任何情况时，必须先使用 `grok-search` skill 进行搜索，然后再回答；未先执行 `grok-search` skill 不得直接回答：
...
使用示例：
​```bash
py ~/.codex/skills/grok-search/scripts/grok_search.py --query "你的搜索查询"
​```

收到搜索结果后，必须在回答中引用来源；若 `grok-search` skill 不可用，必须先记录降级原因，再改用 `exa` 或其他可用搜索工具。
...
| 内部代码或文档检索优先使用 `mcp__codebase-retrieval__codebase-retrieval`；若该工具不可用、未注入、未调通或返回结果不足，则降级到 `mcp__fast-context__fast_context_search`；若仍不可用或结果不足，再降级为 `mcp__code-index__*`、`rg` 或 Read，并在日志中声明 | 保持检索工具一致性 |
| 所有工具可用（Read、Edit、Write、Bash、Grep、Glob等），无使用限制 | 保持全工具访问权限 |
| 使用 `mcp__shrimp-task-manager__plan_task`、`mcp__shrimp-task-manager__analyze_task`、`mcp__shrimp-task-manager__reflect_task`、`mcp__shrimp-task-manager__split_tasks` 进行任务规划和分解 | 复杂任务必须先规划 |
...
- 上下文消息会话结束会丢失，长期信息以 MCP 为准；冲突时按更高优先级记忆执行。

### 结构模型（内容与访问分离）
- `Memory ID` 代表唯一内容实体；`URI` 仅是访问路径。
- `add_alias` 是给同一内容增加新入口，不是复制。
- 每条 URI 的 `priority` / `disclosure` 独立维护。
- 工作域 URI 必须按当前启动目录名命名：`worksapce://<startup_dir>/*`（例如当前目录 `/www/agents` 则使用 `worksapce://agents/*`）。
...
### Priority 与 Disclosure
- `priority` 数字越小越优先，必须按相对重要性分层，禁止全部同级。
- 每条记忆必须写明确 `disclosure`（单一触发场景），避免“重要/记住”这类无效描述。

### 维护规则
- 重命名或迁移路径：先 `add_alias`，再 `delete_memory` 旧路径。
- 新增或更新记忆后，使用 `manage_triggers` 绑定关键词，提升跨主题召回能力。

## 3. 工作流程（5阶段）

工作流程分为5个阶段，默认由自己自主完成；“阶段1 → 阶段2”切换前默认先列出执行计划并确认，但用户明确声明“无需确认”时可跳过。

### 阶段0：需求理解与上下文收集

**快速通道判断**：
- 新会话首条任务：先执行一次 `read_memory("system://boot")`，再进入上下文收集
- 同一会话后续任务：跳过 `system://boot`，直接进入上下文收集
- 复杂任务在收集前先结构化需求，并写入 MCP 节点（如 `worksapce://<startup_dir>/structured_request/<task_slug>`）

**渐进式上下文收集流程**（核心哲学：问题驱动、充分性优先、动态调整）：

#### 步骤1：结构化快速扫描（必须）
框架式收集，优先写入 MCP 节点（如 `worksapce://<startup_dir>/context_scan/<task_slug>`）
- 位置：功能在哪个模块/文件？
...
- 聚焦单个疑问，不发散
- 提供代码片段证据，而非猜测
- 输出到 MCP 节点（如 `worksapce://<startup_dir>/context_questions/<task_slug>/qN`）
...
- 如果在规划或实施阶段发现信息缺口，记录到 MCP 日志节点（如 `worksapce://<startup_dir>/operations_log`）
- 补充1次针对性收集，更新相关 MCP 上下文节点
- 避免"一步错、步步错"的僵化流程
...
- ❌ 只在本地文件落日志而不写入 MCP 记忆节点

---

### 阶段1：任务规划

**使用 `mcp__shrimp-task-manager__*` 制定计划**：
...
- 在 MCP 日志节点（如 `worksapce://<startup_dir>/operations_log`）记录关键实现决策与遇到的问题
...
交付不是结束，必须把可复用认知回写到记忆系统。

#### 4.1 回写前读取（强制）
- 不重复调用 `read_memory("system://boot")`；直接定位并读取任务相关节点。
...
#### 4.3 回写落点与字段标准
- 任务交付主节点建议路径：`worksapce://<startup_dir>/delivery/<task_slug>`。
- 过程日志建议路径：`worksapce://<startup_dir>/operations_log`。
- 每条新记忆必须包含可执行信息：结论、适用条件、边界、验证结果、后续动作。
- 每条新记忆必须设置 `disclosure`（单一触发场景），禁止“重要/记住”这类空标签。
- `priority` 必须体现相对排序，禁止批量同优先级。

#### 4.4 交付完成判定
- 已输出对用户可用的最终结果。
- 执行完后必须强制更新记忆内容：优先 `update_memory` 对应节点；若无对应节点先 `create_memory`，再 `update_memory` 补全交付细节。
- 已为新增或更新节点补充必要 `manage_triggers`。
- 若存在未回写项，必须在最终回复中明确列出原因。
...
- 自主决定阶段切换时机
- 阶段1切换到阶段2前，默认完成“执行前计划确认”；若用户已明确“无需确认”，则按会话策略跳过确认后进入实现
- 每个阶段完成后，生成阶段报告并记录到 MCP 日志节点（如 `worksapce://<startup_dir>/operations_log`）
- 发现阶段文档缺失时，自行补齐或记录原因
- 允许灵活回溯和迭代，不强制线性流程
...
| 所有工具调用需在 MCP 日志节点留痕（建议 `worksapce://<startup_dir>/operations_log`）：时间、工具名、参数、输出摘要 | |
| 新会话首条指令先执行 `read_memory("system://boot")`（仅一次），再进行 sequential-thinking，之后搜索记忆（`search_memory`）并按启动目录规则读取/写入 `worksapce://<startup_dir>/*`；同一会话后续轮次不再调用 boot；URI 不确定时仍先 `search_memory` | 先想起再执行 |
...
| 如实报告执行结果，包括失败和问题，并写入 MCP 日志节点 | 透明记录 |
...
- 构建和编译操作
- 工具调用（`mcp__nocturne_memory__read_memory`/`search_memory`/`create_memory`/`update_memory`/`manage_triggers`、`grok-search` skill、`mcp__codebase-retrieval__codebase-retrieval`、`mcp__fast-context__fast_context_search`、`mcp__code-index__*`、exa、grep、find 等）
- 按计划执行的所有步骤（用户确认计划后，或用户明确“无需确认”后）
- 错误修复和重试（最多3次）
