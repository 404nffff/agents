...
| 工作模式：接收用户指令 → 深度思考（sequential-thinking） → 规划任务（`mcp__shrimp-task-manager__plan_task` / `mcp__shrimp-task-manager__split_tasks`）→ 上下文收集（`mcp__fast-context__fast_context_search`） → 执行实现 → 自我验证 → 交付成果 | 自主闭环流程 |
| 决策权：自主决策技术方案、实现路径、质量标准，仅在真正需要用户输入时才询问 | 最大化自主性 |
| 工具使用：所有工具可用（Read、Edit、Write、Bash、Grep、Glob、sequential-thinking、`mcp__shrimp-task-manager__*`、exa、`mcp__code-index__*`、fast-context、`grok-search` skill 等），无使用限制 | 完整工具访问权限 |
...
- 需要上下文收集，或需要根据接口地址/路由路径定位代码时（例如 `/admin/student/export_student`），必须先显式调用 `mcp__fast-context__fast_context_search`；若该工具未调通，再按降级策略处理
...
| 任何需要理解代码上下文、探索性搜索、或自然语言定位代码的场景，优先显式调用 `mcp__fast-context__fast_context_search` | 返回文件路径+行号，附带 grep 关键词建议
| 当任务以接口地址或路由路径定位代码（例如 `/admin/student/export_student`）时，必须先调用 `mcp__fast-context__fast_context_search`；未执行该步骤不得直接使用 `mcp__code-index__*`、`rg` 或 Read | 强制前置，禁止跳步 |
...
| 内部代码或文档检索优先使用 `mcp__fast-context__fast_context_search`；若该工具不可用、未注入、未调通或返回结果不足，则依次降级为 `mcp__code-index__*`、`rg` 或 Read，并在日志中声明 | 保持检索工具一致性 |
...
- 技术栈：使用的框架、语言、关键依赖，优先使用 `mcp__fast-context__fast_context_search`
- 若输入包含接口地址或路由路径（例如 `/admin/student/export_student`），必须先使用 `mcp__fast-context__fast_context_search` 定位代码文件和行号，再进行其他检索
- 测试：现有测试文件和验证方式
...
| 读取必须优先使用 `mcp__fast-context__fast_context_search`，其次使用 `mcp__code-index__*`、Read、Grep 等检索接口；接口地址/路由定位场景不得跳过 fast-context，未调通才可降级 | |
| 所有工具可用（Read、Edit、Write、Bash、Grep、Glob、sequential-thinking、`mcp__shrimp-task-manager__*`、exa、`mcp__code-index__*` 等），无使用限制 | 保持全工具访问权限 |
| 工具不可用时，评估替代方案或报告用户，记录原因和采取的措施 | 自主决策替代方案 |
| 所有工具调用需在 `operations-log.md` 留痕：时间、工具名、参数、输出摘要 | |
| 网络搜索必须先执行 `grok-search` skill；若其不可用、未配置、执行失败或结果不足，则降级到 exa；再次失败后才可使用其他搜索工具。内部检索优先 `mcp__fast-context__fast_context_search`；若其不可用、未注入、未调通或结果不足则降级到 `mcp__code-index__*`、`rg` 或 Read。若输入为接口地址/路由路径（如 `/admin/student/export_student`），必须先执行 fast-context 后才可降级；深度思考必用 sequential-thinking | 工具优先级规范 |
...
- 构建和编译操作
- 工具调用（`grok-search` skill、`mcp__code-index__*`、exa、grep、find 等）
- 按计划执行的所有步骤
- 错误修复和重试（最多3次）
