...
```
若 `MYSQL_DATABASE_main` 配置了多个库，请追加：`--database <db_name>`。
...
3. 选择多库 profile（显式指定 database）：
```bash
php ~/.codex/skills/mysql-query/scripts/mysql_query.php --profile reporting --database report_db --table report_daily --limit 50
```

4. 降级方案（PHP 不可用时）：
```bash
bash ~/.codex/skills/mysql-query/scripts/mysql_query.sh --profile main --database main_db --table report_daily --limit 50
```
...
6. `MYSQL_DATABASE_<profile>` 支持单库或多库（逗号分隔），例如：`MYSQL_DATABASE_main=rsync,main_db,analytics`
7. 当 `MYSQL_DATABASE_<profile>` 配置了多个库时，必须通过 `--database` 指定目标库，否则脚本会报错
